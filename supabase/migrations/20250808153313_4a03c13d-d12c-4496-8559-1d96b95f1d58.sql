...
create trigger if not exists trg_validate_entry_ownership
...
create policy if not exists "Users can view their own habit entries"
  on public.habit_entries
  for select
  using (auth.uid() = user_id);

create policy if not exists "Users can create their own habit entries"
  on public.habit_entries
  for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own habit entries"
  on public.habit_entries
  for update
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own habit entries"
  on public.habit_entries
  for delete
  using (auth.uid() = user_id);

create trigger if not exists trg_habit_entries_updated_at
